...
# ==========================================================
def require_admin(uid: int):
    # Synchronous helper to avoid repetitive code
    # We actually need async check, so we do it inline in endpoints
    pass
...
async def api_admin_broadcast(text: str = Form(...), init_data: str = Header(None, alias="X-Telegram-Init-Data")):
    ...
    ok = 0
    # Create task to not block response
    async def _send():
        nonlocal ok
        for u in uids:
