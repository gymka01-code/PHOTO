...
from fastapi import FastAPI, File, Form, HTTPException, Request, UploadFile, Header, Body
...
VIP_TIERS = [
    (0,  43200,  3), (3,  39600,  6), (10, 36000,  9),
    (20, 32400, 12), (30, 28800, 15), (50, 25200, 18),
]
...
FAKE_USERS = ["u***r7", "a***2", "m***k9", "p***y4", "t***3", "j***8", "k***5", "s***1", "PhotoNinja_7", "SniperLens_3", "PixelHunter_2", "SnapMaster_5", "crypto_king_77", "usdt_master", "p2p_shark"]
...
async def init_db():
    async with get_db() as db:
        ...
        async with db.execute("SELECT user_id, extra_slots FROM players WHERE extra_slots > 0") as cur:
            ...
            for r in rows:
                for _ in range(r[1]): await db.execute("INSERT INTO user_slots (user_id, is_permanent) VALUES (?, 1)", (r[0],))
            if rows: await db.execute("UPDATE players SET extra_slots = 0")
        ...
        async with db.execute("SELECT COUNT(*) FROM wheel_config") as cur:
            if (await cur.fetchone())[0] == 0:
                for p in DEFAULT_WHEEL_PRIZES: await db.execute("INSERT INTO wheel_config (id, type, val, label, color, chance) VALUES (?,?,?,?,?,?)", (p['id'], p['type'], p['val'], p['label'], p['color'], p['chance']))
...
async def get_active_photo_count(user_id: int) -> int:
    async with get_db() as db:
        async with db.execute("SELECT COUNT(*) FROM photos WHERE user_id=? AND date(datetime(created_at, '+3 hours')) = date(datetime('now', '+3 hours')) AND created_at >= IFNULL((SELECT last_slot_reset FROM players WHERE user_id=?), '1970-01-01')", (user_id, user_id)) as cur: return (await cur.fetchone())[0]
...
async def get_user_total_slot_limit(user_id: int, refs: int) -> int:
    ...
    async with get_db() as db:
        ...
        async with db.execute("SELECT bonus_slots_today FROM players WHERE user_id=?", (user_id,)) as cur:
            ...
            bonus = b_row[0] if b_row and b_row[0] else 0
...
async def get_referral_list(referrer_id: int) -> list:
    async with get_db() as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT r.referred_id, r.created_at, p.username, CASE WHEN p.photos_sold > 0 THEN 1 ELSE 0 END AS is_active FROM referrals r LEFT JOIN players p ON p.user_id = r.referred_id WHERE r.referrer_id = ? ORDER BY r.created_at DESC", (referrer_id,)) as cur: return [dict(r) for r in await cur.fetchall()]
...
async def get_sponsors():
    async with get_db() as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT * FROM sponsors") as cur: return [{"avatar": f"{WEBAPP_URL}/uploads/sponsors/{r['avatar_filename']}" if r["avatar_filename"] else ""} | dict(r) for r in await cur.fetchall()]
...
async def dispatch_support_ticket(uid: int, tkt_id: int, text: str, claimed_by: int = None, img_name: str = None):
    ...
    for aid in target_ids:
        try: 
            if img_name: await bot.send_photo(aid, photo=FSInputFile(SUPPORT_DIR / img_name), caption=caption, parse_mode=ParseMode.HTML, reply_markup=kb)
            else: await bot.send_message(aid, caption, parse_mode=ParseMode.HTML, reply_markup=kb)
        except: pass
...
async def auction_worker():
    while True:
        try:
            ...
            async with get_db() as db:
                db.row_factory = aiosqlite.Row
                async with db.execute("SELECT * FROM photos WHERE status='on_auction' AND sell_at<=?", (datetime.utcnow().isoformat(),)) as cur: due = await cur.fetchall()
                ...
                for ph in due:
                    ...
                    async with db.execute("SELECT photos_sold, referred_by FROM players WHERE user_id=?", (ph["user_id"],)) as cur: u_data = await cur.fetchone()
                    is_first_sale = False
                    if u_data and u_data["photos_sold"] == 0 and ph["user_id"] not in first_sale_users:
                        is_first_sale = True; first_sale_users.add(ph["user_id"])
                    ...
                    async with db.execute("SELECT balance, lang FROM players WHERE user_id=?", (ph["user_id"],)) as cur:
                        if p_row := await cur.fetchone(): notifications.append({"uid": ph["user_id"], "lang": p_row["lang"], "rub": sale_rub, "gross": gross, "net": net, "buyer": buyer, "bal": p_row["balance"]})
...
async def monitor_withdrawals_worker():
    while True:
        ...
        try:
            async with get_db() as db:
                db.row_factory = aiosqlite.Row
                async with db.execute("SELECT id, user_id, amount_usd, warning_sent_at FROM withdrawal_requests WHERE status='pending'") as cur: reqs = await cur.fetchall()
...
async def sponsor_expiry_worker():
    while True:
        ...
        try:
            async with get_db() as db:
                db.row_factory = aiosqlite.Row
                async with db.execute("SELECT * FROM sponsors WHERE expires_at IS NOT NULL AND expires_at <= ?", (datetime.utcnow().isoformat(),)) as cur: expired = await cur.fetchall()
                for sp in expired:
...
async def send_or_edit(obj, text: str, reply_markup=None, parse_mode=ParseMode.HTML):
    if isinstance(obj, CallbackQuery):
        try: await obj.message.edit_text(text, reply_markup=reply_markup, parse_mode=parse_mode)
        except: pass
    else: await obj.answer(text, reply_markup=reply_markup, parse_mode=parse_mode)
...
async def cmd_admin(obj, state: FSMContext=None):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Откройте Админ-Панель в WebApp", web_app=WebAppInfo(url=WEBAPP_URL))]
    ])
    await send_or_edit(obj, "👑 <b>Админ Панель перенесена в приложение!</b>\nОткройте PhotoFlip и нажмите на вкладку Admin (или в меню).", reply_markup=kb)
...
async def admin_native_reply(message: Message):
    ...
    async with get_db() as db:
        ...
        if status == 'open':
            ...
            await db.execute("INSERT INTO support_messages (ticket_id, user_id, text, direction) VALUES (?,?,?, 'system')", (tkt_id, uid, f"👨‍💻 Служба поддержки присоединилась к диалогу."))
...
async def cq_tclaim(cb: CallbackQuery):
    ...
    async with get_db() as db:
        async with db.execute("SELECT user_id, status FROM tickets WHERE id=?", (tkt_id,)) as cur: tkt = await cur.fetchone()
        ...
        await db.execute("INSERT INTO support_messages (ticket_id, user_id, text, direction) VALUES (?,?,?, 'system')", (tkt_id, tkt[0], f"👨‍💻 Служба поддержки присоединилась к диалогу."))
        await db.commit()
...
async def cq_tclose(cb: CallbackQuery):
    ...
    async with get_db() as db:
        ...
        await db.execute("INSERT INTO support_messages (ticket_id, user_id, text, direction) VALUES (?,?,?, 'system')", (tkt_id, uid, "✅ Диалог завершен."))
        await db.commit()
...
async def api_get_player(user_id: int, username: str = "", init_data: str = Header(None, alias="X-Telegram-Init-Data")):
    ...
    if await get_setting("maintenance_mode") == "1":
        if not is_adm: return JSONResponse(status_code=403, content={"error": "maintenance", "end_time": await get_setting("maintenance_end")})
    ...
    async with get_db() as db:
        ...
        if personal:
            try:
                ...
                w_conf = []
                for i, p in enumerate(DEFAULT_WHEEL_PRIZES): w_conf.append({**p, "chance": chances[i]})
            except:
...
async def api_wheel_spin(init_data: str = Header(None, alias="X-Telegram-Init-Data")):
    ...
    cur_chance = 0; prize = prizes[-1]
    for pr in prizes:
        cur_chance += pr["chance"]
        if r <= cur_chance: prize = pr; break
...
async def api_buy_item(item: str = Form(...), init_data: str = Header(None, alias="X-Telegram-Init-Data")):
    ...
    async with get_db() as db:
        ...
        if not p or p["balance"] < price: raise HTTPException(400, detail="Недостаточно средств" if item == "slots" else "Not enough funds")
...
async def api_promo_activate(req: PromoReq, init_data: str = Header(None, alias="X-Telegram-Init-Data")):
    ...
    async with get_db() as db:
        ...
        if promo["uses"] >= promo["max_uses"]: raise HTTPException(400, detail="Promo code usage limit reached")
        async with db.execute("SELECT 1 FROM promo_uses WHERE user_id=? AND code=?", (uid, code)) as cur:
            if await cur.fetchone(): raise HTTPException(400, detail="You already used this code")
...
async def api_support_send(
    text: str = Form(""), file: UploadFile = File(None), init_data: str = Header(None, alias="X-Telegram-Init-Data")
):
    uid = verify_webapp_data(init_data)
    await check_maintenance(uid)
    text = text.strip()
...
async def api_admin_data(init_data: str = Header(None, alias="X-Telegram-Init-Data")):
    ...
    async with get_db() as db:
        ...
        async with db.execute("SELECT COUNT(*) FROM withdrawal_requests WHERE status='pending'") as cur: total_wds = (await cur.fetchone())[0]

    return {"sponsor_mode": sponsor_mode, "recent_chats": recent_chats, "stats": {"users": total_users, "wds": total_wds}}
...
async def api_admin_chat_send(
    target_uid: int = Form(...), text: str = Form(""), file: UploadFile = File(None), init_data: str = Header(None, alias="X-Telegram-Init-Data")
):
    ...
    async with get_db() as db:
        ...
        if not tkt: await db.execute("INSERT INTO support_messages (ticket_id, user_id, text, direction) VALUES (?,?,?, 'system')", (tkt_id, target_uid, f"👨‍💻 Служба поддержки присоединилась к диалогу."))
