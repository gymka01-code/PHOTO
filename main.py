...
EXTRA_SLOT_PRICE_USD   = 5.0 # Цена за доп. слот
...
def get_total_slot_limit(refs: int, extra: int) -> int:
    return vip_slot_limit(refs) + extra
...
async def get_sponsors() -> list[dict]:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT * FROM sponsors WHERE expires_at > datetime('now')") as cur:
...
async def auction_worker():
    while True:
        ...
        await asyncio.sleep(10) # Ускорено до 10 сек
        ...
        try:
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                ...
                for req in requests:
                    ...
                    all_subbed = True
                    for sp in sponsors:
                        if not await is_subscribed_to_channel(sp['channel_id'], uid):
                            all_subbed = False
                            break
...
async def expire_sponsors_worker():
    while True:
        await asyncio.sleep(3600)
        try:
            async with aiosqlite.connect(DB_PATH) as db:
                ...
                for sp in expired:
                    for aid in await get_admin_ids():
                        try: await bot.send_message(aid, f"⚠️ Спонсор <b>{sp['name']}</b> ({sp['channel_id']}) истёк и был удалён.", parse_mode=ParseMode.HTML)
                        except: pass
...
async def _process_start(target: Message, user, args: str = ""):
    ...
    slots     = get_total_slot_limit(ref_count, player.get("extra_slots", 0))
...
async def cb_check_sub(cb: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT OR REPLACE INTO quests (user_id, channel_id, completed) VALUES (?,?,1)", (user_id, REQUIRED_CHANNEL_ID))
        await db.commit()
...
# ADMIN COMMANDS - SPONSORS
@dp.message(Command("addsponsor"))
async def cmd_addsponsor(message: Message, command: CommandObject):
    if not await is_admin(message.from_user.id): return
    try:
        parts = [p.strip() for p in (command.args or "").split("|")]
        ch_id, days, url, name, desc = parts[0], int(parts[1]), parts[2], parts[3], parts[4]
        expires = (datetime.utcnow() + timedelta(days=days)).isoformat()
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("INSERT OR REPLACE INTO sponsors (channel_id, url, name, description, expires_at) VALUES (?,?,?,?,?)", (ch_id, url, name, desc, expires))
            await db.commit()
        await message.answer(f"✅ Спонсор <b>{name}</b> добавлен до {expires[:10]}.", parse_mode=ParseMode.HTML)
    except Exception:
        await message.answer("❌ Ошибка. Формат:\n`/addsponsor -100... | 7 | https://t.me/... | Имя | Описание канала`", parse_mode=ParseMode.MARKDOWN)

@dp.message(Command("delsponsor"))
async def cmd_delsponsor(message: Message, command: CommandObject):
    if not await is_admin(message.from_user.id): return
    ch_id = (command.args or "").strip()
    async with aiosqlite.connect(DB_PATH) as db:
...
async def cmd_sponsors(message: Message):
    if not await is_admin(message.from_user.id): return
    sps = await get_sponsors()
    if not sps: return await message.answer("Спонсоров нет.")
    text = "📢 <b>Активные спонсоры:</b>\n\n"
    for s in sps:
        text += f"• <b>{s['name']}</b> ({s['channel_id']})\n🔗 {s['url']}\n📝 {s['description']}\n⏳ До: {s['expires_at'][:10]}\n\n"
...
async def lifespan(app: FastAPI):
    ...
    t4 = asyncio.create_task(expire_sponsors_worker())
    yield
...
async def api_get_player(user_id: int, username: str = ""):
    ...
    extra = player.get("extra_slots", 0)
    ...
    return {
        ...
        "slot_limit": get_total_slot_limit(ref_count, extra),
        "slot_price": EXTRA_SLOT_PRICE_USD,
...
async def api_buy_slot(request: Request):
    user_id = (await request.json()).get("user_id")
    player = await get_player(user_id)
    if not player: raise HTTPException(404, "Player not found")
    if (player["balance"] or 0) < EXTRA_SLOT_PRICE_USD:
        raise HTTPException(400, "Not enough balance")

    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE players SET balance=balance-?, extra_slots=extra_slots+1 WHERE user_id=?", (EXTRA_SLOT_PRICE_USD, user_id))
        await db.commit()
    return {"success": True, "new_balance": round(player["balance"] - EXTRA_SLOT_PRICE_USD, 2)}
...
async def api_upload(user_id: int = Form(...), username: str = Form(""), files: List[UploadFile] = File(...)):
    ...
    slot_limit = get_total_slot_limit(ref_count, player.get("extra_slots", 0))
