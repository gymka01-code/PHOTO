...
from aiogram.types import (import asyncio
import logging
import os
import random
import urllib.parse
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        # Включаем WAL режим для ускорения работы с базой (избавляет от database is locked)
...
async def auction_worker():
    while True:
        try:
            ...
            # Быстрая транзакция для работы с БД
            async with aiosqlite.connect(DB_PATH) as db:
                db.row_factory = aiosqlite.Row
                ...
                for photo in due:
                    ...
                    # Собираем данные для уведомлений, чтобы не держать базу заблокированной
                    async with db.execute("SELECT balance, lang FROM players WHERE user_id=?", (photo["user_id"],)) as cur:
                        p_row = await cur.fetchone()
                        if p_row:
                            notifications.append({
                                "uid": photo["user_id"], "lang": p_row["lang"], 
                                "sale_rub": sale_rub, "gross": gross, 
                                "net": net, "buyer": buyer, "balance": p_row["balance"]
                            })
                
                if due:
                    await db.commit()

            # Отправка сообщений вне транзакции БД (чтобы не блочить WebApp)
            for n in notifications:
                try: 
                    await bot.send_message(n["uid"], tr(n["lang"], "sold", rub=int(n["sale_rub"]), gross=n["gross"], commission=round(n["gross"]-n["net"],2), net=n["net"], buyer=n["buyer"], balance=round(n["balance"], 2)), parse_mode=ParseMode.HTML)
                except Exception: 
                    pass
...
async def cq_c_usr_actions(cb: CallbackQuery, state: FSMContext):
    ...
    if not uid:
        try:
            await cb.answer("⏳ Сессия устарела. Вернитесь назад и введите ID заново.", show_alert=True)
        except Exception:
            pass
        return
...
async def cq_sp_manage(cb: CallbackQuery):
    ...
    if not sp: 
        try:
            return await cb.answer("Спонсор не найден", show_alert=True)
        except Exception:
            return
...
async def cq_tview(cb: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute("SELECT t.id, t.user_id, t.status, p.username FROM tickets t LEFT JOIN players p ON t.user_id=p.user_id WHERE t.id=?", (tkt_id,)) as cur:
            tkt = await cur.fetchone()
        if not tkt or tkt["status"] != "open": 
            try: return await cb.answer("Уже взят или закрыт.", show_alert=True)
...
async def cq_wdok(cb: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    try: await cb.answer("✅ Отмечено как выплаченное", show_alert=True)
    except Exception: pass
    await cq_wd(cb)
...
async def cq_wdrej_do(cb: CallbackQuery):
    ...
    if mode == "notify":
        ...
        try: await cb.answer("❌ ОТКЛОНЕНА, юзер уведомлен.", show_alert=True)
        except Exception: pass
    else:
        try: await cb.answer("❌ ОТКЛОНЕНА тихо.", show_alert=True)
        except Exception: pass
...
async def api_upload(user_id: int = Form(...), username: str = Form(""), files: List[UploadFile] = File(...)):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        for idx, (orig_name, raw) in enumerate(files_data):
            ...
            # АСИНХРОННОЕ сохранение картинки на диск, чтобы не блокировать бота!
...
# --- NON-BLOCKING WEBHOOK (ИЗБАВЛЯЕТ ОТ ПЕРЕПОДКЛЮЧЕНИЙ И ЛАГОВ БОТА) ---
@app.post(WEBHOOK_PATH)
async def bot_webhook(request: Request):
    from aiogram.types import Update
    data = await request.json()
    
    # Моментально отвечаем серверу Telegram, чтобы он не слал дубли и не выдавал таймауты
